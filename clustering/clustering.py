...
class Clustering:
    ...
    def clusterSpectral(self, trajectories, clusters=8):
        ...
        # a = zip(list(Eval), list(Evec.T))
        #
        # b = sorted(list(a), key=operator.itemgetter(0), reverse=True)
        #
        # Eval, Evec = zip(*b)

        Eval, Evec = zip(*sorted(zip(Eval, Evec.T), key=operator.itemgetter(0), reverse=True))
